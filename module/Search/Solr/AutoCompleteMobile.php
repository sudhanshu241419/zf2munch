...
class AutoCompleteMobile {
    ...
    public function getAutocomplete($req) {
        ...
        $this->searchUrls = new SearchUrlsMobile();
    ...
    private function prepareResFacetData($cui_url, $top_url = '', $name_url = '',$sec='') {
        ...
        if (empty($sec))
        {
            ...
            if ($urlResponse['status_code'] == 200) {
                ...
                if ($this->debug == 1) {//debug
                    ...
                    $retData['data'][] = array('item' => SearchHelpers::getDebugUrl($cui_url), 'type' => 'cui_url');
                }
            } else {
            ...
            if (count($retData) < 5 && $top_url != '') {
                ...
                if ($urlResponse['status_code'] == 200) {
                    ...
                    if ($this->debug == 1) {//debug
                        $retData['data'][] = array('item' => SearchHelpers::getDebugUrl($top_url), 'type' => 'top_url');
                    }
                }
            }
        }
        ...
        if (count($retData) < 5 && $name_url != '') {
            ...
            //debugurl
            if ($this->debug == 1) {//debug
                $retData['data'][] = array('item' => SearchHelpers::getDebugUrl($name_url), 'type' => 'name_url');
            }
        }
    ...
    private function prepareFoodFacetData($cui_url, $top_url = '', $name_url = '') {
        ...
        if (count($retData) < 1 && $name_url != '') {
            ...
            if ($this->debug == 1) {
                $retData['data'][] = array('item' => SearchHelpers::getDebugUrl($cui_url), 'type' => '');
            }
        }

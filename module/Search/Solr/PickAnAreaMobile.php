...
class PickAnAreaMobile {
    ...
    public function __construct() {
        $this->_solr_url = StaticOptions::getSolrUrl() . 'hbr/hbsearch?';
        $this->solr_select_url = StaticOptions::getSolrUrl() . 'hbr/select?';
    }
    ...
    public function getLandmarksData($req) {
        ...
        //$groupPart = 'q=*:*&rows=1000&fl=res_neighborhood,borough,nbd_lat,nbd_long&group=true&group.field=borough&sort=borough+asc,res_neighborhood+asc&group.limit=50&';
        
        $searchUrl = new SearchUrlsMobile();
        ...
        if ($this->debug) {
            $response ['params'] = $req;
            $response ['landmarks'] [] = array('url' => SearchHelpers::getDebugUrl($url), 'landmark' => '', 'latitude' => '', 'longitude' => '');
        }
    ...
    public function getTopNeighborhoods($req) {
        ...
        if ($this->debug) {
            $response ['landmarks'] [] = array('url' => SearchHelpers::getDebugUrl($url));
        }
        return $response;
    }

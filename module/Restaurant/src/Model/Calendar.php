...
class Calendar extends AbstractModel {
    ...
    protected $_db_table_name = 'Restaurant\Model\DbTable\CalendarTable';
    protected $_primary_key = 'id';

    protected function getMappedDay($abbr) {
        return isset(StaticOptions::$dayMapping [$abbr]) ? StaticOptions::$dayMapping [$abbr] : '';
    }
    ...
    public function isRestaurantTakeout($restaurant_id, $is_rest_open = false, $has_takeout) {
        ...
        foreach (StaticOptions::getRestaurantTakeoutTimeSlots($restaurant_id, $date) as $t) {
            if ($t ['status'] == 1) {
    ...
    public function restaurantDeliveryTimeDiff($restaurant_id, $has_delivery) {
        ...
        if ($has_delivery == 1) {
            ...
            $currentDayDelivery = StaticOptions::getPerDayDeliveryStatus($restaurant_id, $date);
    ...
    public function getDeliveryTakeoutTimmer($restaurantId = false, $hasDeliveryTakeout = false, $orderType = false) {
        ...
        if ($restaurantId) {
            $timezoneformat = StaticOptions::getTimeZoneMapped(array('restaurant_id' => $restaurantId));
    ...
    public function getFirstSlot($orderType, $hasDeliveryTakeout, $restaurantId, $currentDate) {
        ...
        if ($orderType == 'order') {
            $slots = StaticOptions::getRestaurantOrderTimeSlots($restaurantId, $currentDate);
        } elseif ($orderType == 'takeout') {
            $slots = StaticOptions::getRestaurantTakeoutTimeSlots($restaurantId, $currentDate);
        }
        $firstDeliverySlot = false;
    ...
    public function getNextDaySlot($nextDateTime,$restaurant_id){
        ...
        $currentDayDelivery = StaticOptions::getPerDayDeliveryStatus($restaurant_id, $nextDateTime->format("Y-m-d"));
    ...
    public function getResWeekCalData($restaurant_id) {
     ...
     public function restaurantOpenCloseTime($restaurantid){
        $currentDateTime = \MCommons\StaticOptions::getRelativeCityDateTime(array(
                    'restaurant_id' => $restaurantid
                ))->format(StaticOptions::MYSQL_DATE_FORMAT);
        $day = date("D",strtotime($currentDateTime));
        $keys = array_keys(StaticOptions::$dayMapping, $day);       
        $select = new Select ();
        $select->from($this->getDbTable()->getTableName());
        $select->columns(array(
            'open_time',

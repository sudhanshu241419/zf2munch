...
class Restaurant extends AbstractModel {
	...
	public function getRestaurantLocation($restaurant_id = 0) {
		...
		$restaurantLocation = $this->getDbTable ()->setArrayObjectPrototype ( 'ArrayObject' )->getReadGateway ()->selectWith ( $select );
    ...
    public function getHispanicRestaurant($promotionId,$currentDate){
        ...
        $select->join ( array ('pre' => 'promotion_restaurant_event'),
            'pre.restaurant_id = restaurants.id',
             array('restaurant_id'),
            $select::JOIN_INNER
        );

        $select->join (array('p' => 'promotions'),
           'pre.promotionId=p.promotionId',
            array(

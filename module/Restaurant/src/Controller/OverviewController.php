...
class OverviewController extends AbstractRestfulController {
    ...
    public function get($restaurant_id = 0) {
        ...
        if ($imageGallery) {
            $cover = array('image' => $response ['cover_image'], 'title' => '', 'type' => 'restaurant');
            array_unshift($imageGallery, $cover);
            $response ['galleries'] = array_merge($imageGallery, $userUploadedImage);
        } else {
            $response ['galleries'][] = array_merge(array('image' => $response ['cover_image'], 'title' => '', 'type' => 'restaurant'), $userUploadedImage);
        }
        ...
        $response['activity'] = array('message' => '', 'image' => array());
        if ($isLoggedIn) {

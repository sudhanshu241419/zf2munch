...
class WebReservationController extends AbstractRestfulController {
...
//            }
        ...
        $userReservationModel->host_name = (isset($data['host_name']) && !empty($data['host_name']))?$data['host_name']:PROTOCOL.SITE_URL;
...
//        else
        ...
        if ($userId) {
            ...
            $userPoints = strval($totalPoints[0]['points'] - $redeemPoint);
...
//                $userModel = new User();
                ...
                if(isset($awardPoint_reservation['points'])){
                    ...
                    $orderpoint = isset($data['order_point'])?$data['order_point']:0;
                    if ($userTransaction == 0) {                        
                        $userPoints = $userPoints + $orderpoint;
                        $userPointTable = $reservationPoints['points'] +$orderpoint;
                    } else {
                        $userPoints = $userPoints + $reservationPoints['points'] + $orderpoint;
                        $userPointTable = $reservationPoints['points'] + $orderpoint;
                    }
...
//            }
        ...
        $settingData = '';
        $timeslot = array();
        $timeslot [0] = $data ['date'];
        $timeslot [1] = $data ['time'];
        ...
        $webUrl = $userReservationModel->host_name;
        if (!isset($data['order_id'])) {
...
//                        'url' => $webUrl,
//                        'isRestaurantRegister' => ($userReservationModel->status == 4) ? 1 : 0,
//                        'receipt_no' => $userReservationModel->receipt_no,
//                        'restaurant_address' => $restaurantAddress,
//                        'user_instruction' => $userReservationModel->user_instruction,
//                    ),
//                    'subject' => 'Your Reservation has been Reserved',
//                    'template' => '05_Your-Reservation-has-been-reserved',
//                    'layout' => 'email-layout/default_new'
//                );
//            } 
            
            if($userReservationModel->status == 1){
                $layout = ($userReservationModel->host_name==PROTOCOL.SITE_URL)?'email-layout/default_new':'email-layout/ma_default';
                $emailData = array(
                    'receiver' => array(
                        $data ['email']
                    ),
        ...
        if ($sendMailToRestaurant) { 
            ...
            $restaurantAddress = $restaurantfunction->restaurantAddress($data ['restaurant_id']);
            $config = $this->getServiceLocator ()->get ( 'Config' ); 
            $webUrl = $userReservationModel->host_name; 
            $layout = 'email-layout/default';
            $sender = $data ['restaurant_name'];
            ...
            $user_function->sendMailsToRestaurant ( $sendMailToMunchAdoCustomerArray,$sender ); 
        }
...
//                );
//
//                $notificationJsonArray = array('first_name' => ucfirst($userReservationModel->first_name), 'is_friend' => 0, 'username' => ucfirst($userName), 'user_id' => $userId, 'reservation_id' => $reservation['id'], 'reservation_status' => $userReservationModel->status, 'restaurant_id' => $data['restaurant_id'], 'restaurant_name' => ucfirst($data ['restaurant_name']));
...
//                    'first_name' => ucfirst($userReservationModel->first_name)
//                );
//                $notificationJsonArray = array('first_name' => ucfirst($userReservationModel->first_name), 'is_friend' => 0, 'username' => ucfirst($userName), 'user_id' => $userId, 'reservation_id' => $reservation['id'], 'reservation_status' => $userReservationModel->status);
//                $response = $userNotificationModel->createPubNubNotification($notificationArray, $notificationJsonArray);
//                $pubnub = StaticOptions::pubnubPushNotification($notificationArray);
//            } else {
         ...
         //   }
            ...
            */
            ...
            $pubnub = StaticOptions::pubnubPushNotification($notificationArray);
            /**
             * Push To Pubnub For cms dashboard
             */
            ...
            # Pubnub Notification to user : when the order/reservation is confirmed by the restaurant or by the CRM team
//            if ($userReservationModel->status == 4) {
//                $notificationMsg = 'Reservation reserved at ' . ucfirst($data ['restaurant_name']) . '. Good get.';
//                $channel = "mymunchado_" . $userId;
//                $notificationArray = array(
...
//                $pubnub = StaticOptions::pubnubPushNotification($notificationArray);
//
//                #Push To Pubnub For Restaurant:When a user makes a reservation with pre-ordering
//
//                $notificationMsg = "You have a new pre-paid reservation. Review it now.";
//                $channel = "dashboard_" . $data ['restaurant_id'];
//                $notificationArray = array(
//                    "msg" => $notificationMsg,
//                    "channel" => $channel,
...
//            }
        ...
        if ($userId) {
            ...
            $isAlreadyBookedmark = $restaurantBookmark->isAlreadyBookmark(array(
                'type' => 'bt',
                'restaurant_id' => $data ['restaurant_id'],
                'user_id' => $userId
            ));
        ...
        ########start of send sms to unregister user on second transaction############
        //if (!$userId) {
            //$userFunctions = new UserFunctions ();
            //$sendSmsonTransactionCount = $userFunctions->sendSmsonTransactionCount($data ['email']);
        //}
        ########END of send sms to unregister user on second transaction############
        ...
        $clevertapData = array(
            ...
            "email"=>$data ['email'],
            "identity"=>$data ['email'],
            "reservation_date"=>StaticOptions::getFormattedDateTime($userReservationModel->time_slot, 'Y-m-d H:i', 'Y-m-d'),
            ...
            "type"=>"reservation",
            "res_date_time"=>$userReservationModel->time_slot,
            "host_url"=>isset($data['host_name'])?$data['host_name']:PROTOCOL.SITE_URL
    ...
    public function update($id, $data) {
        ...
        if ($type == 'modify') {
            ...
            if(strtotime($reservedOn) > strtotime($data ['time_slot'])){
                throw new \Exception('Reservation time has been expired! Try again');
            }
...
//                }
                ...
                if ($totalSeatCount > $restaurantAllocatedSeat) {
                    ...
                    $largeGroupBackwordImpactSeatCount = 0;
                    if ($dineinCalendarsDetail['dinningtime_small'] > TIME_INTERVAL) {
                        $smallGroupBackwordSeatCount = $reservationFunction->checkSmallBackword($dineinCalendarsDetail, $data, $userReservatioModel->id);
                        $smallGroupForwordSeatCount = $reservationFunction->checkSmallForword($dineinCalendarsDetail, $data, $userReservatioModel->id);
                    }
                    //echo "smallGroupForwordSeatCount-".$smallGroupForwordSeatCount . ":" ;
                    //calculate total seat occupied by large group
                    if ($dineinCalendarsDetail['dinningtime_large'] > TIME_INTERVAL) {
                        $largeGroupBackwordSeatCount = $reservationFunction->checkLargeBackword($dineinCalendarsDetail, $data, $userReservatioModel->id);
                        $largeGroupForwordSeatCount = $reservationFunction->checkLargeForword($dineinCalendarsDetail, $data, $userReservatioModel->id);
                    }

                    //echo "largeGroupForwordSeatCount-".$largeGroupForwordSeatCount . ":" ;
                    //caclculate carry forward reservation on future time slots
                    if ($dineinCalendarsDetail['dinningtime_large'] > (2 * TIME_INTERVAL)) {
                        $largeGroupBackwordImpactSeatCount = $reservationFunction->checkLargeBackwordImpact($dineinCalendarsDetail, $data, $userReservatioModel->id);
                    }
            ...
            $userReservatioModel->reserved_on = $reservedOn;
            ...
            if ($timeSlotMatch === 0 && $seatMatch === 0) {
                ...
                //if ($sendMail == true) {
                    $sendModifyReservationMail = array(
                        ...
                        'variables' => array(
                            ...
                            'question_feel_free' => '<tr><td style="padding-bottom:30px;padding-top:20px;color:#867f7c;font-size:14px;font-family:arial;font-style:italic;">If you have any further questions feel free to email us (The Munch Ado Crew) at <a href="mailto:wecare@munchado.com" style="text-decoration:none;color: #867f7c; font-weight: bold; outline: none;">WeCare@MunchAdo.com</a> or give us a call at <a href="tel:1-888-345-1303" style="font-weight:bold;color:#867f7c;text-decoration:none">1-888-345-1303</a>.</td></tr>',
                        ),
                    ...
                    foreach ($invitationData as $data) {
                        ...
                        if (!empty($data ['to_id'])) {
                            ...
                            $notificationArray = array(
                                ...
                                'reservation_id' => $id,
                                'reservation_status' => $existData['status'],
                                'friend_name'=>ucfirst($existData ['first_name']),
                                'friend_id'=>$existData ['user_id'],
                                'restaurant_name'=>ucfirst($existData ['restaurant_name'])
                            );
                          
                            $notificationJsonArray = array('restaurant_name'=>ucfirst($existData ['restaurant_name']),'friend_name'=>ucfirst($existData ['first_name']),'friend_id'=>$existData ['user_id'],'is_friend' => 0, 'username' => ucfirst($userName), 'reservation_id' => $id, 'reservation_status' => $existData['status'], 'user_id' => $data ['to_id'], 'restaurant_id' => $existData['restaurant_id'], 'restaurant_name' => ucfirst($existData ['restaurant_name']));
                            $response = $userNotificationModel->createPubNubNotification($notificationArray, $notificationJsonArray);
                            $pubnub = StaticOptions::pubnubPushNotification($notificationArray);
                        }
                    }
                ...
                if ($userReservatioModel->user_id) {
                    ...
                    $notificationJsonArray = array('reservation_id' => $userReservatioModel->id,'first_name' => ucfirst($userReservatioModel->first_name),'restaurant_name'=>ucfirst($existData ['restaurant_name']),'is_friend' => 0, 'username' => ucfirst($userName), 'reservation_id' => $id, 'reservation_status' => $existData['status'], 'user_id' => $userReservatioModel->user_id);
                    $response = $userNotificationModel->createPubNubNotification($notificationArray, $notificationJsonArray);
                    $pubnub = StaticOptions::pubnubPushNotification($notificationArray);
                }
                ...
                $notificationArray = array(
                    ...
                    "restaurantId" => $existData ['restaurant_id'],
                    'curDate' => $currentDate,
                    'is_friend' => 0, 'username' => ucfirst($userName), 'reservation_id' => $id, 'user_id' => $userReservatioModel->user_id
                );
                $notificationJsonArray = array('is_friend' => 0, 'username' => ucfirst($userName), 'reservation_id' => $id, 'user_id' => $userReservatioModel->user_id);
                //$response = $userNotificationModel->createPubNubNotification($notificationArray, $notificationJsonArray);
                //$pubnub = StaticOptions::pubnubPushNotification($notificationArray);
            }
    ...
    public function get($reservationId = 0) {
        ...
        $joins [] = array(
            'name'=>array('r'=>'restaurants'),
            'on'=>'r.id = user_reservations.restaurant_id',
            'columns'=>array('min_partysize'),
            'type'=>'left'
        );                
        ...
        if (!empty($reservation)) {
            ...
            $reservation ['currentDate'] = date('Y-m-d', strtotime($currentDate));
            $dateTime = explode(" ", $reservation ['time_slot']);
            $reservation ['date'] = $dateTime [0];
            $reservation ['time'] = $dateTime [1];
            $reservation1 = StaticOptions::getRestaurantReservationTimeSlots($reservation ['restaurant_id'], $reservation ['date']);
            $finalReserve = array();
            foreach ($reservation1 as $reserve) {
                if ($reserve ['status'] == 1) {
                    $finalReserve [] = $reserve ['slot'];
                }
                unset($reserve ['slot']);
                unset($reserve ['status']);
            }
            $reservation ['timeslots'] = $finalReserve;
    ...
    public function delete($id) {
        ...
        if (!empty($invitationData)) {
            foreach ($invitationData as $data) {
                ...
                if (!empty($data ['to_id'])) {
                    ...
                    //$notificationJsonArray = array(ucfirst($reservationData ['first_name']),'friend_id'=>$reservationData ['user_id'],'is_friend' => 1, 'username' => ucfirst($FirstName), 'reservation_id' => $id, 'user_id' => $data ['to_id']);
                    $response = $userNotificationModel->createPubNubNotification($notificationArray, $notificationJsonArray);
                    $pubnub = StaticOptions::pubnubPushNotification($notificationArray);
                }
            }
        ...
        if (!empty($reservationData ['user_id'])) {
            ...
            $notificationArray = array(
                ...
                'curDate' => $currentDate,
                'is_friend' => 0,
                'username' => ucfirst($resData ['user_name']),
                'reservation_id' => $id,
            ...
            $notificationJsonArray = array('is_friend' => 0, 'username' => ucfirst($resData ['user_name']), 'reservation_id' => $id, 'user_id' => $reservationData ['user_id'], 'reservation_status' => $userReservatioModel->status, 'restaurant_id' => $reservationData ['restaurant_id'], 'restaurant_name' => ucfirst($restaurant->restaurant_name));
            $response = $userNotificationModel->createPubNubNotification($notificationArray, $notificationJsonArray);
            $pubnub = StaticOptions::pubnubPushNotification($notificationArray);
        }
        ...
        $dashboardArray = array(
            "msg" => $msg,
            "channel" => $channel,
            "userId" => isset($reservationData ['user_id'])?$reservationData ['user_id']:0,
            "type" => 'reservation',
            "restaurantId" => $reservationData ['restaurant_id'],
            'curDate' => $currentDate,
            'is_friend' => 0, 
        ...
        $notificationJsonArray = array('is_friend' => 0, 'username' => ucfirst($resData ['user_name']), 'reservation_id' => $id, 'user_id' => $reservationData ['user_id'], 'reservation_status' => $userReservatioModel->status);
        $userNotificationModel->createPubNubNotification($dashboardArray, $notificationJsonArray);
        StaticOptions::pubnubPushNotification($dashboardArray);
        $pointSourceModel = new PointSourceDetails ();
        $userPointModel = new UserPoint ();
        $sl = $this->getServiceLocator();
        $config = $sl->get('Config');
        $pointID = isset($config ['constants'] ['point_source_detail']) ? $config ['constants'] ['point_source_detail'] : array();
        if (!empty($reservationData ['user_id']) && $reservationData['status'] == 4) {

...
class RestaurantDistanceCalculationFunction {

    public function checkRestaurantDistance($restaurant_id, $address) {
        $deliveryData = array();
        $restaurantModel = new Restaurant();
        $restaurantLatLon = $restaurantModel->findRestaurant(array(
            'columns' => array(
                'latitude',
                'longitude',
                'delivery_area'
        ...
        if (!empty($restaurantLatLon)) {
            $restaurantLat = $restaurantLatLon->latitude;
            $restaurantLong = $restaurantLatLon->longitude;
            $maxDelDistance = $restaurantLatLon->delivery_area;
        }
    ...
    public function parseGoogleResponse($gResponse) {
        ...
        $address_types_street = $config['constants']['address_types_street'];
        $address_types_neighbourhood = $config['constants']['address_types_neighbourhood'];
        $address_types_zip = $config['constants']['address_types_zip'];
        $address_types_city = $config['constants']['address_types_city'];
        // Get Latitude
        ...
        $types = $gResponse->results [0]->types[0];  
        if(in_array($types, $address_types_street)){
            $latLon['types'] = 'street';
        }elseif(in_array($types, $address_types_neighbourhood)){
            $latLon['types'] = 'neighbourhood';
        }elseif(in_array($types, $address_types_zip)){
            $latLon['types'] = 'zip';
        }elseif(in_array($types, $address_types_city)){
            $latLon['types'] = 'city';
        }

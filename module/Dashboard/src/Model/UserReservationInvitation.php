...
class UserReservationInvitation extends AbstractModel
{
    ...
    protected $_db_table_name = 'Dashboard\Model\DbTable\UserReservationInvitationTable';
...
//		));
    ...
    public function getUserInvitations($reservationId,$flag = 0) {
        $select = new Select();
        $select->from($this->getDbTable()
                        ->getTableName());
        $select->columns(array(
            'total_invite' => new Expression('COUNT(id)'),
        ));
        $where = new Where ();
        $where->equalTo('reservation_id', $reservationId);
        $where->equalTo('msg_status', self::ACCEPTED);
        if($flag == 1){
            $where->notEqualTo('to_id', 'null');
        }
        $select->where($where);
        ...
        if (!empty($data)) {
            return $data['total_invite'];
        } else {
            return 0;
        }
    }
    public function getUserAllInvitations($data) {
        $select = new Select();
        $select->from($this->getDbTable()
                        ->getTableName());
        $select->columns(array(
        ...
        $where = new Where ();
        $where->equalTo('reservation_id', $data['id']);

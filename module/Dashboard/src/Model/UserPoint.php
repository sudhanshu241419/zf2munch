...
class UserPoint extends AbstractModel {
    ...
    protected $_db_table_name = 'Dashboard\Model\DbTable\UserPointTable';
...
//    		/var_dump($select->getSqlString($this->getPlatform('READ')));die;
    ...
    public function updatePointEntry($userId, $reservationId, $points,$restName) {
        $data = [];
        $select = new Select();
        $select->from($this->getDbTable()
                        ->getTableName());
        ...
        $where = new Where ();
        $where->equalTo('user_id', $userId);
        $where->equalTo('ref_id', $reservationId);
        $select->where($where);
        //var_dump($select->getSqlString($this->getPlatform('READ')));die;
        ...
        if (!empty($responce)) {
            ...
            $data['points_descriptions'] = "You snagged a spot at " . $restName . "! This calls for a celebration, here are " . $points . " points!";
    ...
    public function updateUserPointStatus($userId, $reservationId) {
        $data = [];
        $select = new Select();
        $select->from($this->getDbTable()
                        ->getTableName());
    ...
    public function getGuestPoints($id,$restId) {
        $select = new Select();
        $select->from($this->getDbTable()
                        ->getTableName());
        $select->columns(array(
        ...
        ), 'rs.restaurant_id =  user_points.restaurant_id and rs.user_id = user_points.user_id and user_points.created_at >= rs.date', array(
        ), $select::JOIN_INNER);
        $where = new Where();
        $where->equalTo('user_points.user_id', $id);
        $select->where($where);
       // pr($select->getSqlString($this->getPlatform('READ')),true);die;
        $userPoints = $this->getDbTable()
                        ->setArrayObjectPrototype('ArrayObject')
                        ->getReadGateway()
                        ->selectWith($select)->current();
    ...
    public function getUserPointsData($userIds, $restId, $filter_val) {
        //pr($userIds,1);
        $ids = [];
        $select = new Select();
        $select->from($this->getDbTable()
        ...
        $having->expression('sum(points) >= ?', $filter_val);
        $where = new Where();
        $where->in('user_points.user_id', $userIds);
        $select->where($where);

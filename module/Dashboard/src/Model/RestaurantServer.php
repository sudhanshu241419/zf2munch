...
class RestaurantServer extends AbstractModel {
    ...
    public function earlyBirdSpecial($userId = 0, $restId = 0, $orderDelivery = false, $earlySpecialDays = 0) {
        $select = new Select();
        ...
        if ($orderDelivery && !empty($data)) {
            $registrationDate = $data['date'];
            $date1 = new \DateTime($registrationDate);
            $date2 = new \DateTime($orderDelivery);
            $interval = $date1->diff($date2);
            if ($interval->days <= $earlySpecialDays) {
                return true;
            }
        }
        return false;
    }
 public function getGuestbook($restId,$ids=[0],$flag) {
        $select = new Select();        
        $select->from($this->getDbTable()->getTableName());
        $select->columns(array('user_id','restaurant_id','code','created_at'=>'date'));        
        $select->join(array(
            'rs' => 'restaurants'
        ), 'rs.id =  restaurant_servers.restaurant_id', array(
            'restaurant_name'
    ...
    public function userRegisterWithDineAndMore($restId,$userId) {
        $select = new Select();        
        $select->from($this->getDbTable()->getTableName());
        ...
        $select->where($where);       
        //var_dump($select->getSqlString($this->getPlatform('READ')));   
        $count = $this->getDbTable()->setArrayObjectPrototype('ArrayObject')->getReadGateway()->selectWith($select)->current();
        if($count){
            return $count['count'];
        }else{
            return 0;

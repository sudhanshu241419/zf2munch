...
class UserAddressController extends AbstractRestfulController {
    ...
    function getList() {
        ...
        $userAddress = new UserAddress ();
        $response = $userAddress->getUserAddressDetail($user_id,$city_name);
        $resturentId = $this->getQueryParams('restid');
        
        foreach ($response as $key => $val) {
            if ($resturentId) {
                $res_can_deliver = \Search\CityDeliveryCheck::canDeliver($resturentId, $val['latitude'], $val['longitude']);

...
class UserPoints extends AbstractModel {
	...
	public $user_id;
	public $status = 1;
	protected $_db_table_name = 'User\Model\DbTable\UserPointsTable';
	...
	public function UserPointsDetails($user_id = 0) {
        ...
        $select->join(array(
            ...
            ), 'pd.id = user_points.point_source',
            array(
            'activity_type'=>'name',
    ...
    public function UserLastActivity($user_id){
        ...
        $select->join(array('ps' => 'point_source_detail'),
            'ps.id = user_points.point_source',
             array('acitvity_identifier' => 'identifier'),
             $select::JOIN_INNER);

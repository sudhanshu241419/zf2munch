...
class User extends AbstractModel {
    ...
    public $last_login;
    public $access_token;
    protected $_db_table_name = 'User\Model\DbTable\UserTable';
    ...
    public function create_user_auth($user_data) {
        ...
        $auth_data['created_at'] = StaticOptions::getDateTime()->format(StaticOptions::MYSQL_DATE_FORMAT);
        $auth_data['updated_at'] = StaticOptions::getDateTime()->format(StaticOptions::MYSQL_DATE_FORMAT);
        try {
    ...
    public function getUserDetailWithStatistics($userId, $currentUserId = null) {
        ...
        if ($UserDetail) {
            ...
            $UserDetail['created_at'] = StaticOptions::getFormattedDateTime($UserDetail['created_at'], 'Y-m-d H:i:s', 'M d, Y');
     ...
     * @throws \Exception
     */
    public function getUserReferralCode($user_id, $device = 'false') {
        //pr($user_id,1);
        $this->getDbTable()->setArrayObjectPrototype('ArrayObject');
        $options = array(
        ...
        $writeGateway = $this->getDbTable()->getWriteGateway();
        $referral_code = UserFunctions::generateUserReferralCode($user_id);
        $data = array('referral_code' => $referral_code, 'referral_ext' => null);
        $rowsAffected = $writeGateway->update($data, array('id' => $user_id));
        if (!$rowsAffected == 1) {

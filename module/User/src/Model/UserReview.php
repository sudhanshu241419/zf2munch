...
class UserReview extends AbstractModel {
	...
	protected $_db_table_name = 'User\Model\DbTable\UserReviewTable';
    ...
    public function getWinnersUserRestaurantIds(){
        ...
        $select->join ( array ('uri' => 'user_review_images'),
            'uri.user_review_id = user_reviews. id',
             array('id','created_at','image_path'=>new Expression ('image_url')),
            $select::JOIN_INNER
        );
        $select->where ( array (
				'uri.image_status' => '4',  
    ...
    public function getMenuWinnersUserRestaurantIds(){
     $select = new Select ();
        ...
        $select->join ( array ('uri' => 'user_menu_reviews'),
            'uri.user_review_id = user_reviews. id',
             array('id','image_path'=>new Expression ('image_name')),
            $select::JOIN_INNER
        );
        $select->join ( array ('r' => 'restaurants'),
            'r.id= user_reviews.restaurant_id',
             array('rest_code'),
            $select::JOIN_INNER
        );
        $select->where ( array (
    ...
    public function reviewStatus($reviewId = 0) {
        ...
        $review = $this->getDbTable ()->setArrayObjectPrototype ( 'ArrayObject' )->getReadGateway ()->selectWith ( $select )->toArray();

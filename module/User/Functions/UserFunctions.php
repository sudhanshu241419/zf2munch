...
class UserFunctions {
    ...
    public function getHomeReservationDetail($record) {
        ...
        $date = StaticOptions::getFormattedDateTime($record ['time_slot'], 'Y-m-d H:i:s', 'D, M d, Y');
        $time = StaticOptions::getFormattedDateTime($record ['time_slot'], 'Y-m-d H:i:s', 'h:i A');
        $timeForSlot = StaticOptions::getFormattedDateTime($record ['time_slot'], 'Y-m-d H:i:s', 'H:i:s');
    ...
    public function getIndividualOrder($flag = null) {
        ...
        if (isset($output) && !empty($output)) {
            ...
            foreach ($output as $key1 => $value) {
                $dateData [] = StaticOptions::getFormattedDateTime($value ['delivery_time'], 'Y-m-d H:i:s', 'Y-m-d H:i:s');
            }
            ...
            $orderTime = StaticOptions::getFormattedDateTime($record ['created_at'], 'Y-m-d H:i:s', 'Y-m-d');
            $orderDelTime = StaticOptions::getFormattedDateTime($record ['delivery_time'], 'Y-m-d H:i:s', 'Y-m-d');
            $compOrderTime = StaticOptions::getFormattedDateTime($record ['created_at'], 'Y-m-d H:i:s', 'Y-m-d H:i:s');
            $currentDate = StaticOptions::getFormattedDateTime($date, 'Y-m-d H:i:s', 'Y-m-d');

            if ($orderTime == $currentDate) {
                $record ['order_time'] = $userOrderModel->dateToString($compOrderTime, $date);
            } else {
                $record ['order_time'] = StaticOptions::getFormattedDateTime($record ['created_at'], 'Y-m-d H:i:s', 'M d, Y');
            }
            if ($orderDelTime == $currentDate) {
                $record ['deliver_on'] = StaticOptions::getFormattedDateTime($record ['delivery_time'], 'Y-m-d H:i:s', 'h:i A');
            } else {
                $record ['deliver_on'] = StaticOptions::getFormattedDateTime($record ['delivery_time'], 'Y-m-d H:i:s', 'M d, Y \a\t\ h:i A');
            }
    ...
    public function getPendingOrder() {
        ...
        if (!$preOrderRecord) {
            ...
            $orderTime = StaticOptions::getFormattedDateTime($preOrderRecord ['created_at'], 'Y-m-d H:i:s', 'Y-m-d');
            $compOrderTime = StaticOptions::getFormattedDateTime($preOrderRecord ['created_at'], 'Y-m-d H:i:s', 'Y-m-d H:i:s');
            $currentDate = StaticOptions::getFormattedDateTime($date, 'Y-m-d H:i:s', 'Y-m-d');

            if ($orderTime == $currentDate) {
                $preOrderRecord ['order_time'] = $userOrderModel->dateToString($compOrderTime, $date);
            } else {
                $preOrderRecord ['order_time'] = StaticOptions::getFormattedDateTime($preOrderRecord ['created_at'], 'Y-m-d H:i:s', 'M d, Y');
            }
            $preOrderRecord ['deliver_date'] = StaticOptions::getFormattedDateTime($preOrderRecord ['delivery_time'], 'Y-m-d H:i:s', 'M d, Y');
            ...
            $preOrderRecord ['deliver_on'] = StaticOptions::getFormattedDateTime($preOrderRecord ['delivery_time'], 'Y-m-d H:i:s', 'h:i A');
    ...
    public function normalLogin($data) {
        ...
        $tokeModel = new \Auth\Model\Token();
...
//            throw new \Exception($error);
    ...
    public function googleLogin($data) {
        ...
        if ($userDetail) {
            ...
            $domain = $this->checkDomain($userDetail ['email']);
//            if ($domain === 'edu') {
...
//            } else {
            ...
            if ($loyalityCode) {
                $salesData['tags'] = array("Registration_form", "Dine_and_More",$this->restaurant_name);
                $salesData['restaurant_name'] = $this->restaurant_name;
                $salesData['restaurant_id'] = $this->restaurantId;
                $salesData['dine_more'] = "Yes";
                $salesData['point'] = 200;
                $salesData['totalpoint'] = (int) $this->userTotalPoint($userModel->id);
                ...
                $salesData ['restaurant_id'] = "";
                $salesData['restaurant_name'] = "";
...
//                $mailData = array('recievers' => $userModel->email, 'template' => $template, 'layout' => $layout, 'variables' => $variables, 'subject' => $subject);
...
//        if (!isset($data ['displayName']) && empty($data ['displayName'])) {
//            throw new \Exception('Display name is not valid', 400);
//        }

//        if (!isset($data ['first_name']) && empty($data ['first_name'])) {
//            throw new \Exception('User first name is not valid', 400);
//        }
...
//            } else {
            ...
            $salesData['owner_email'] = 'no-reply@munchado.com';
            $salesData['restaurant_name'] = ($loyalityCode) ? $this->restaurant_name : "";
            $salesData ['restaurant_id'] = ($this->loyaltyCode) ? $this->restaurantId : "";
            $salesData['tags'] = ($loyalityCode) ? array("Registration_form", "Dine_and_More",$this->restaurant_name) : array("Registration_form");
            $salesData['contact_ext_event_type'] = "OTHER";
            $salesData['redeempoint'] = 0;
            if ($loyalityCode) {
                $salesData['point'] = 200;
    ...
    public function registerUser($data) {
        try {
            ...
            if (isset($data['referral_code']) && !empty($data['referral_code'])) {
                $this->isReferralCodeValid($data['referral_code']);
            }
           ...
           // pr($data,1);
            ...
            $domain = $this->checkDomain($userloginModel->email);
...
//            } else {
            ...
            $salesData ['restaurant_id'] = ($this->loyaltyCode) ? $this->restaurantId : "";
            $salesData['tags'] = ($this->loyaltyCode) ? array("Registration_form", "Dine_and_More",$this->restaurant_name) : array("Registration_form");
            $salesData['contact_ext_event_type'] = "OTHER";
            $salesData['redeempoint'] = 0;
            if ($loyalityCode) {
                $salesData['point'] = 200;
            ...
            $response1['total_point'] = $salesData['totalpoint'];
            return $response1;
        } catch (\Exception $e) {
    ...
    public function getAddFieldCalendar($value) {
        $value ['start_date'] = (!empty($value ['reserved_on']) && $value ['reserved_on'] != null) ? $value ['reserved_on_date_readable'] . " " . StaticOptions::getFormattedDateTime($value ['reserved_on'], 'Y-m-d H:i:s', 'h:i A') : null;
    ...
    public function timeLater($start, $end, $type = null) {
        ...
        if ($sdate > $edate) {
            if ($time >= 0 && $time <= 59) {
                ...
                if ($preday [0] > 7) {
                    $timeshift = StaticOptions::getFormattedDateTime($start, 'Y-m-d H:i:s', 'M d, Y');
                }
            }
        }
        return $timeshift;
    ...
    public function sendMails($data, $sender = array()) {
        ...
        // $c = StaticOptions::getServiceLocator()->get('config');
    ...
    public function getAllocatedPoints($key) {
        $pointsModel = new PointSourceDetails ();
    ...
    public function givePoints($points, $userId, $message = null, $refId = null, $takeThisUserId = false) {
        $userIdFromSession = isset($userId) ? $userId : StaticOptions::getUserSession()->getUserId();
        $locationData = StaticOptions::getUserSession()->getUserDetail('selected_location', array());
        $currentDate = $this->userCityTimeZone($locationData);
    ...
    public function sendOrderMail($data, $status, $userId, $restaurant_id) {
        ...
        if(isset($restaurantMail ['email']) && !empty($restaurantMail ['email'])){
    ...
    public function sendRegistrationEmail($data) {
        ...
        // StaticOptions::resquePush($emailData);
        ...
        if (!empty($friendsToMake)) {
            foreach ($friendsToMake as $friends) {
                ...
                $dataArray = array(
                    ...
                    'created_on' => StaticOptions::getDateTime()->format(StaticOptions::MYSQL_DATE_FORMAT),
                    'status' => 1
                );
    ...
    public function createSettings($userId, $ns = false) {
        ...
        $userSettingsModel->create($userId, $ns);
    }
...
//            $this->givePoints($points, $inviteeId, $message); 
    ...
    public function checkRestaurantStatus($restaurant_id, $deleveryTime) {
        if (!empty($restaurant_id) && !empty($deleveryTime)) {
            $endDate = StaticOptions::getRelativeCityDateTime(array(
                        'restaurant_id' => $restaurant_id
                            ), StaticOptions::getDateTime()->format(StaticOptions::MYSQL_DATE_FORMAT), 'Y-m-d H:i:s');
            $deleveryTimes = StaticOptions::getAbsoluteCityDateTime(array(
                        'restaurant_id' => $restaurant_id
    ...
    public function sendMailsToServiceProvider($data) {
        ...
        $subject = $data ['subject'];
        $mailToServiceProvider = StaticOptions::sendMailToServiceProvider($sender, $sendername, $recievers, $template, $layout, $data ['variables'], $data ['subject'], $data['receiverCc'], $data['attachment']);
    ...
    public function preOrderReservationMail($data, $preOrderReservation, $userId = '', $orderPass) {
        ...
        $variables['order_pass_through'] = $orderPass;
        $variables['reservationDate'] = StaticOptions::getFormattedDateTime($timeslot [0], 'Y-m-d', 'D, M d, Y');
        $variables['reservationtime'] = StaticOptions::getFormattedDateTime($timeslot [1], 'H:i', 'h:i A');
        $variables['orderType'] = "Pre-paid reservation";
        $variables['reservationTime'] = StaticOptions::getFormattedDateTime($timeslot [0], 'Y-m-d', 'D, M d, Y') . " " . StaticOptions::getFormattedDateTime($timeslot [1], 'H:i', 'h:i A');
...
//            $userTemplate = 'Its_a_date_And_time_And_food';
    ...
    public function takePoints($points, $userId, $refId = null, $takeThisUserId = false) {
        $userIdFromSession = StaticOptions::getUserSession()->getUserId();
        if ($userIdFromSession && !$takeThisUserId) {
            $userId = $userIdFromSession;
        }
        $userPointsModel = new UserPoint ();
...
//        }
        ...
        $userLoginDetail = array();
        $userLoginDetail ['first_name'] = $userModel->first_name;
        $userLoginDetail ['last_name'] = $userModel->last_name;
        $userLoginDetail ['phone'] = $userModel->phone;
        $userLoginDetail ['newsletter_subscribtion'] = $userModel->newsletter_subscribtion;
        $userLoginDetail ['profile_image_url'] = $userModel->display_pic_url;
        $userLoginDetail ['email'] = $userModel->email;
...
//                    $template = 'five-doller-on-registration';
//                    $layout = '05_registration';
//                    $subject = '$'.PROMOCODE_FIRST_REGISTRATION.' for You';
//                    $variables = array(
//                        'userName' => $userName                                    
//                    );
//                    $mailData = array('recievers' => $userEmail, 'template' => $template, 'layout' => $layout, 'variables' => $variables, 'subject' => $subject);
//                    $this->emailSubscription($mailData);
//                }
            }
    ...
    public function myAvatar($avatarId) {
        ...
        $options = array(
            'column' => array('id', 'action_count', 'total_earned', 'date_earned'),
            'where' => array('user_id' => $userId, 'avatar_id' => $avatarId),
        );
        $userAvatarDetail = $userAvatar->find($options)->toArray();
    ...
    public function getAllAvatar($avatarId = false) {
        ...
        if ($avatarId) {
            $options['where'] = array('id' => $avatarId);
        }

        $avatarModel->getDbTable()->setArrayObjectPrototype('ArrayObject');
        if ($avatarModel->find($options)->toArray()) {
            $allAvatar = $avatarModel->find($options)->toArray();
        }
        return $allAvatar;
    }

    public function getMyAvatar($allAvatar = array()) {
        $myAvatar = array();
        if (!empty($allAvatar)) {
            $userAvatar = new Model\UserAvatar();
            $i = 0;
    ...
    public function isReviewUsefullCount($reviewId) {
        $feedback = new \Restaurant\Model\UserFeedback();
        ...
        $feedback->getDbTable()->setArrayObjectPrototype('ArrayObject');
...
//        }
    ...
    public function associateInvitation($open_page_type = false, $refId = false, $userId = false, $userEmail = false) {
        ...
        if (!empty($refId) && $refId) {
            ...
            if (!empty($open_page_type) && $open_page_type == 3) {
                ...
                $session = StaticOptions::getUserSession();
                $token = StaticOptions::getUserSession()->token;
    ...
    public function saveReferredUserInviterData($user_id, $referral_code) {
        ...
        $conn = $user->getDbTable()->getWriteGateway()->getAdapter()->getDriver()->getConnection();
        ...
        try {
           ...
           // pr($ur->getInviterReferralExist($user_id, $restaurantId, $inviter_info['id']),1);
            ...
            if($isFriend==0){
                ...
                $userFriendsInvitation = new UserFriendsInvitation();
                $userFriendsInvitation->getDbTable()->getWriteGateway()->insert(array(
                ...
                $user_friends->getDbTable()->getWriteGateway()->insert(array(
                    'user_id' => $user_id,
...
//                $notificationMsg = ucfirst($current_user_data['first_name']) . " accepted your friend request. Soon you two will be finishing each other's sandwich orders.";
//                $channel = "mymunchado_" . $inviter_info['id'];
//                $notificationArray = array(
//                    "msg" => $notificationMsg,
//                    "channel" => $channel,
//                    "userId" => $user_id,
//                    "friend_iden_Id" => $inviter_info['id'],
//                    "type" => 'friendship',
//                    "restaurantId" => '0',
//                    'curDate' => $currentDateTime,
//                    'username' => ucfirst($current_user_data['first_name'])
//                );
//                $notificationJsonArray = array('user_id' => $user_id, 'username' => ucfirst($current_user_data['first_name']));
               
//                $userNotificationModel->createPubNubNotification($notificationArray, $notificationJsonArray);
//                StaticOptions::pubnubPushNotification($notificationArray);

                $inviteeName = $inviter_info['first_name'] . ' ' . $inviter_info['last_name'];
                $feed = array(
                    'user_id' => $user_id,
                    "friend_id" => $inviter_info['id'],
                    'inviter' => ucfirst($inviteeName),
                    'user' => ucfirst($current_user_data['first_name'])
                );
                $replacementData = array('inviter' => ucfirst($inviteeName));
                $otherReplacementData = array('user' => ucfirst($current_user_data['first_name']), 'inviter' => ucfirst($inviteeName));
                //$activityFeed = $commonFunction->addActivityFeed($feed, 54, $replacementData, $otherReplacementData);
                ...
                $replacementData = array('user_name' => ucfirst($current_user_data['first_name']));
                $otherReplacementData = array();

                //$activityFeed = $commonFunction->addActivityFeed($feed, 61, $replacementData, $otherReplacementData);
            }
    ...
    public function inviteFriends($userId, $userName, $currentDate, $email, $data, $baseUrl, $sender = array()) {
        ...
        $config = $sl->get('Config');
        $webUrl = PROTOCOL . $config['constants']['web_url'];
        // $acceptLink = $baseUrl . DS . 'wapi' . DS . 'user' . DS . 'accepted' . DS . $insertData . '?token=' . StaticOptions::getUserSession()->token;
    ...
    public function isReferralCodeValid($referral_code) {
        $userdetail = new User();
    ...
    public function formatLastLoginDate($start, $end, $type = null) {
        ...
        if ($sdate > $edate) {
            if ($time >= 0 && $time <= 59) {
                ...
                $timeshift = $preday [0] . ' days' . " " . $type;
                if ($preday [0] > 7) {
                    $timeshift = StaticOptions::getFormattedDateTime($end, 'Y-m-d H:i:s', 'M d, Y');
                }
            }
        }
        return $timeshift;
    ...
    public function getUserPromocodeDetails() {
        $userPromocodesModel = new \Restaurant\Model\UserPromocodes();
        $userPromocodesModel->getDbTable()->setArrayObjectPrototype('ArrayObject');
        $joins = array();
        $joins [] = array(
            'name' => array('p' => 'promocodes'),
            'on' => new \Zend\Db\Sql\Expression('p.id = promo_id AND p.status = 1'),
            'columns' => array('start_on', 'end_date', 'promocodeType', 'discount', 'discount_type', 'minimum_order_amount', 'slots', 'days', 'deal_for', 'title', 'description'),
            'type' => 'right');
        $options = array('columns' => array('user_id', 'user_promocode_id' => 'id'), 'where' => array('reedemed' => 0, 'user_id' => $this->userId), 'order' => 'p.start_on', 'joins' => $joins);
    ...
    public function checkExistingCodeWithUser() {        
        ...
        $resServer->user_id = StaticOptions::getUserSession()->getUserId();
    ...
    public function registerRestaurantServer() {
        $resServer = new Model\RestaurantServer();
        $resServer->user_id = ($this->userId) ? $this->userId : StaticOptions::getUserSession()->getUserId();
    ...
    public function userRegistrationWithSmsWeb() {
        ...
        if (!empty($userDetail)) {
            ...
            StaticOptions::getUserSession()->setUserId($userDetail['id']);
    ...
    public function smsForRegisterUser($resquedata, $userSmsData) {
        ...
        if ($isExistUserWithCode['0']['id'] == 0) {
            ...
            $restaurantUrl = StaticOptions::getBitlyShortUrl($restaurantUrl);
...
//       pr($mailData,1);
    ...
    public function getUserDealData($currentDate, $userId, $isMobile=false) {              
        ...
        $dealOptions = array(
            ...
            'where' => array('ud.user_id' => $userId, 'restaurant_deals_coupons.status' => 1, 'restaurant_deals_coupons.user_deals' => 1, 'ud.availed' => 0),
            'joins' => $joins,
        ...
        if (!empty($userDeal)) {
            ...
            foreach ($userDeal as $key => $val) {
                //$slotsArray = explode(",", $val['slots']);
                //$daysArray = explode(",", $val['days']);
...
//                    if ($currentDateTimeUnixTimeStamp < $dealsStartDateTimeUnixTimeStamp) {
//                        unset($userDeal[$key]);
//                    }
                    ...
                    $resDetails = $restaurantDetailModel->findRestaurant(array('where' => array('id' => $val['restaurant_id'])))->toArray();
    ...
    public function createQueue($data, $class) {
        StaticOptions::resquePush($data, $class);
    }
    ...
    public function sendServerRegisterMails($data, $sender = array()) {
        ...
        $template = "email-template/" . $data ['template'];
        $layout = (isset($data ['layout'])) ? $data ['layout'] : 'email-layout/default';
        StaticOptions::sendMail($senderEmail, $senderName, $recievers, $template, $layout, $data ['variables'], $data ['subject']);
    }
    ...
    public function restaurantTaged($restaurantId = false) {
        ...
        if (!empty($tagsDetails)) {
            ...
            return $restaurant->findByRestaurantId(
                            array(
                                'columns' => array('restaurant_name', 'rest_code','restaurant_logo_name'),
    ...
    public function dineAndMoreAwardsDuringOrder($awardType){
     ...
     public function sendMunchadoCareerMails($data, $sender = array()) {
       $recievers = array(
        ...
        $template = "email-template/" . $data ['template'];
        $layout = (isset($data ['layout'])) ? $data ['layout'] : 'email-layout/default';
        return StaticOptions::sendCareerMail ( $data['sender'], $data['sender_name'], $recievers, $template, $layout, $data ['variables'], $data ['subject'] );
    }
    ...
    public function clevertapUploadProfile($data){
        ...
        if ($data['loyalitycode'] || $data['source'] === "sms" || $data['source'] === 'enrl') {                
            ...
            $cleverTap['earned _dollar'] = StaticOptions::convertPointToDollar(200);
            ...
            $cleverTap['earned_points'] = 100;
            $cleverTap['redeemed_points'] = 0;
            $cleverTap['earned _dollar'] = StaticOptions::convertPointToDollar(100);
    ...
    public function getRestOrderFeatures($restId){
        $restaurantDetailModel = new Restaurant();
        return $restaurantDetailModel->findRestaurant(array('columns'=>array('delivery','takeout','reservations'),'where' => array('id' => $restId)))->toArray();
    }
    ...
    public function getPromocode($promocode,$restaurantId, $orderamount=false){
        if ($promocode && $restaurantId) {
            ...
            $userSession = StaticOptions::getUserSession();

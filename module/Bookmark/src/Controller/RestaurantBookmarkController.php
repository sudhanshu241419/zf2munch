    ...
    public function create($data) {
        ...
        if ($isRestExists) {
            $restaurant = $restModel->findByRestaurantId(array(
                'column' => array('restaurant_name','rest_code','restaurant_image_name'),
...
//                 $notificationMsg='You know you want '.$restaurant['restaurant_name'].'.';
//               $channel = "mymunchado_" . $session->getUserId();
//               $notificationArray = array(
//                    "msg" => $notificationMsg,
//                    "channel" => $channel,
...
//                   'btype'=>1
//                );
//                $notificationJsonArray = array('btype'=>1,'is_food'=>0,'user_id'=>$session->getUserId(),'first_name'=>ucfirst($userDetail['first_name']),'restaurant_id' => $data['restaurant_id'],'restaurant_name'=>ucfirst($restaurant['restaurant_name']));
//                $pub = $userNotificationModel->createPubNubNotification($notificationArray,$notificationJsonArray);        
//                $pubnub = StaticOptions::pubnubPushNotification($notificationArray);
            }elseif($data ['type']==='bt'){                
                 $replacementData = array('restaurant_name'=>$restaurant['restaurant_name']);

...
class Authenticate {
	...
	protected function isValidToken() {
		...
		if($redisCache) {
			...
			if($redisCache->hasItem($tokenValue)) {
				...
				$tokenDetails = new \Auth\Model\Token();
				$tokenDetails->exchangeArray($tokenDetailsData);
			}
		} else {
			$authTable = $this->_serviceLocator->get ( 'Auth\Model\DbTable\AuthTable' );
			$authTable->setArrayObjectPrototype ( 'Auth\Model\Token' );
	...
	protected function updateTokenTimestamp() {
		...
		if($redisCache) {
			$tokenDetails = new \Auth\Model\Token();
			$tokenModel = $tokenDetails->findToken($this->_tokenValue);
			if(!$tokenModel) {

...
class StaticOptions {
    ...
    public static function setUserAgent($userAgent) {
        if (isset($userAgent->value)) {
            if (preg_match('/Mozilla/i', $userAgent->value) || preg_match('/Chrome/i', $userAgent->value) || preg_match('/Safari/i', $userAgent->value)) {
                static:: $_userAgent = "ws";
            } elseif (preg_match('/Android/i', $userAgent->value)) {
                static:: $_userAgent = "android";
            } elseif (preg_match('/iOS/i', $userAgent->value)) {
    ...
    public static function getUserAgent() {
        if (!static:: $_userAgent) {
            throw new \Exception("Unable to get user agent. Please set the instance of header for user agent first");
        }
    ...
    public static function getDbReadAdapter() {
        if (!static::$_db_read_adapter) {
            static::setDbReadAdapter(static::getServiceLocator()->get('MCommons\Db\Adapter\ReadAdapter'));
        }
    ...
    public static function getDbWriteAdapter() {
        if (!static::$_db_write_adapter) {
            static::setDbWriteAdapter(static::getServiceLocator()->get('MCommons\Db\Adapter\WriteAdapter'));
        }
    ...
    public static function getTimeZoneMapped(array $options = array()) {
        ...
        if (isset($options['state_code']) && $options['state_code']) {
            ...
            $city->getDbTable()->setArrayObjectPrototype('ArrayObject');
        ...
        if (isset($options['restaurant_id']) && $options['restaurant_id']) {
            ...
            $select->where(array(
                'restaurants.id' => $options['restaurant_id']
            ));
            $resturantModel->getDbTable()->setArrayObjectPrototype('ArrayObject');
            $timeZoneResult = $resturantModel->getDbTable()
                    ->getReadGateway()
                    ->selectWith($select)
                    ->current();
    ...
    public static function getRestaurantTakeoutTimeSlots($restaurant_id, $date, $input_datetime_format = 'Y-m-d', $output_datetime_format = 'H:i') {
        ...
        foreach (static::$timeSlots as $slot) {
            ...
            /*
             * Following condition has been written for 24 hours slot to takeout on 12 AM on the basis on bug id 42311
             */
            if ($restaurantDetailsFunctions->isRestaurantOpenTwentyFourHours($restaurant_id)) {
                $finalSlots[] = $slotDateTime;
            } else {
                foreach ($mergedSlots as $ocSlots) {
                    if (!empty($ocSlots)) {
    ...
    public static function sendMail($sender, $sendername, $recievers, $template, $layout, $variables, $subject) {
        ...
        foreach ($recievers as $reciever) {
            $settingUserDb = new \User\Model\User();
    ...
    public static function getSolrUrl() {
        $config = self::getServiceLocator()->get('config');
        if (isset($config['constants']['solr'])) {
            $protocol = $config['constants']['solr']['protocol'];
            $host = $config['constants']['solr']['host'];
            $port = $config['constants']['solr']['port'];
            $context = $config['constants']['solr']['context'];
    ...
    public static function pubnubPushNotification($message) {
        ...
        $difKey = array('userId' => 'user_id', 'restaurantId' => 'restaurant_id');
        $config = self::getServiceLocator()->get('config');
        $pubnub = new \Pubnub($config['constants']['pubnub']['PUBNUB_PUBLISH_KEY'], // PUBLISH_KEY
                $config['constants']['pubnub']['PUBNUB_SUBSCRIBE_KEY'], // SUBSCRIBE_KEY
                "", // SECRET_KEY
                false); // SSL_ON?
        $info = $pubnub->publish(array(
            'channel' => $message['channel'], // 'mymunchado_' . self::getUserSession ()->token, // REQUIRED Channel to Send
            'message' => $message // REQUIRED Message String/Array
        ));
        if (isset($message['msg'])) {
            $notificationModel = new \User\Model\UserNotification();
            //$user_current_notification = $notificationModel->getCurrentNotification($message['userId'], 1, 'one', $message['curDate']);
            ...
            $pubnub->publish(array(
                'channel' => 'ios_' . $message['channel'], // 'mymunchado_' . self::getUserSession ()->token, // REQUIRED Channel to Send
                'message' => $user_current_notification, // REQUIRED Message String/Array  
                'aps' => array('alert' => $message['msg'], 'badge' => intval($count))
            ));
            $pubnub->publish(array(
                'channel' => 'android_' . $message['channel'], // 'mymunchado_' . self::getUserSession ()->token, // REQUIRED Channel to Send
                'message' => $user_current_notification, // REQUIRED Message String/Array  
                'data' => array('summary' => $message['msg'], 'badge' => intval($count))
            ));
        }
    }
    ...
    public static function resquePush($data = array(), $class = "") {
        ...
        if (empty($class) || $class == "SendEmail") {
            ...
            if (isset($config['resque-service']) && $config['resque-service']) {
                ...
                StaticOptions::sendMail($data['sender'], $data['sendername'], $data['receivers'], $data['template'], $data['layout'], $data['variables'], $data['subject']);
            }
    ...
    public static function getPerDayDeliveryStatus($restaurantId = false, $requestedDate = false) {
        ...
        if ($restaurantId) {
            ...
            $calendarDay = array_search($currentDay, StaticOptions::$dayMapping);
            $calendar = new Calendar();
    ...
    public static function getResDistanceInMiles($res_id, $lat, $lng) {
        ...
        return StaticOptions::latLogDistanceCalculation($data['latitude'], $data['longitude'], $lat, $lng);
    }
...
//        }
    ...
    public static function uploadCareerFile($files, $path, $dirname) {
        ...
        if (!empty($files)) {
            foreach ($files as $fkey => $file) {
                if (!empty($file['name'])) {
                    $valid = StaticOptions::validateCareerFile($file);
                    if ($valid['status']) {
...
//                    pr($files->$fkey,1);
    ...
    public static function resquePDF($data = array(), $class = "") {
        ...
        if (empty($class) || $class == "SendEmail") {
            ...
            return StaticOptions::sendMailPDF($data['sender'], $data['sendername'], $data['receivers'], $data['template'], $data['layout'], $data['variables'], $data['subject']);
        } else {
            return '';
        }
    }

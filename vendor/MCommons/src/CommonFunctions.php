...
//            pr($difference);
    ...
    public function getUserHistory($userId = null) {
        ...
        $reserveModel = new UserReservation ();
        $conditions = array(
            'userId' => $userId,
            'group' => 'groupby',
            'currentDate' => $currentDate
...
//        }
    ...
    public function getUserHistoryBackup($userId = null) {
        ...
        $reserveModel = new UserReservation ();
        $reserveModel->getDbTable()->setArrayObjectPrototype('ArrayObject');
        $options = array(
            'columns' => array(
        ...
        $created_at = StaticOptions::getUserSession()->getUserDetail('created_at');
    ...
    public function getUserHistoryForMob($userId = null) {
        ...
        $userModel = new \User\Model\User();
        $userTips = new \User\Model\UserTip();
        $session = StaticOptions::getUserSession();
        $locationData = $session->getUserDetail('selected_location');
        $currentDate = $userFunctions->userCityTimeZone($locationData);
        if (!$userId) {
        ...
        $options = array(
            'columns' => array(
                new Expression("COUNT(*) AS total_tip"),
                'user_id',
                'status'
            ),
            'where' => array(
        ...
        $userTips->getDbTable()->setArrayObjectPrototype('ArrayObject');        
        ...
        $options = array(
            'columns' => array(
                new Expression("COUNT(*) AS total_tryit"),
                'user_id'
            ),
            'where' => array(
                'user_id' => $userId,
        ...
        $menuBookmarkModel->getDbTable()->setArrayObjectPrototype('ArrayObject');
        $count ['tryit'] = $menuBookmarkModel->find($options)->toArray();

       $userOrderModel = new UserOrder ();
//        $options = array(
//            'columns' => array(
//                new Expression("COUNT(*) AS totalOrder"),
//                'user_id'
//            ),
//            'where' => array(
//                'user_id' => $userId
...
//        $userOrderModel->getDbTable()->setArrayObjectPrototype('ArrayObject');
        ...
        $options = array(
            'columns' => array(
                new Expression("COUNT(DISTINCT restaurant_id) AS total_beenthere"),//new Expression("COUNT(*) AS total_beenthere"),
                'user_id'
            ),
            'where' => array(
                'user_id' => $userId,
        ...
        $restaurantBookmarkModel->getDbTable()->setArrayObjectPrototype('ArrayObject');
    ...
    public function getActivityFeedType($activityTypeId = false){
        ...
        if($activityTypeId){
            $activityFeedTypeModel = new ActivityFeedType();
    ...
    public function replaceParticulerKeyValueInArray(&$userDineRestaurant){
        if(!empty($userDineRestaurant)){            
            array_walk ( $userDineRestaurant, function (&$key) { $key["code"] = substr($key["code"], 0, 1).$key["restaurant_id"]."00"; } );
        }
    }

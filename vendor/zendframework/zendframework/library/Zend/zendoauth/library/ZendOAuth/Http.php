...
class Http
{
    ...
    {  
        ...
        try {
            ...
            throw new Exception\InvalidArgumentException(sprintf('Error in HTTP request: %s', $e->getMessage()), null, $e);
        }
        if ($response !== null) {
    ...
    protected function _assessRequestAttempt(\Zend\Http\Response $response = null)
    {
        switch ($this->_preferredRequestScheme) {
            ...
            case OAuth::REQUEST_SCHEME_POSTBODY:
                ...
                break;
            default:
                throw new Exception\RuntimeException('Could not retrieve a valid Token response from Token URL:' . ($response !== null ? PHP_EOL . $response->getBody() : ' No body - check for headers'));
        }
    }
    ...
    protected function _toAuthorizationHeader(array $params, $realm = null)
    {
        ...
        foreach ($params as $key => $value) {
            ...
            $headerValue[] = Http\Utility::urlEncode($key) . '="' . Http\Utility::urlEncode($value) . '"';
        }
